...
async function handleClickBrowse() {
  ...
  if(allowOpenDialog()) {
    ...
    try {
      ...
      Logging.log("MpegStitching.logic.js handleClickBrowse() newState:", newState)

      this.setState(newState)

      Logging.log("MpegStitching.logic.js this.state:", this.state)
    }
    catch(err) {
      store.dispatch(action(ALLOW_OPEN_DIALOG))
      Logging.logError("Error in mpegConversion.logic.handleClickBrowse():", err)
    }
  }
  else {
